...
class TestQueryIncidentsUseCase:
    ...
    def test_execute_no_filters(self, query_incidents_use_case, mock_incident_repository):
        """Given: No filters, When: Execute, Then: Return all incidents."""
        # Arrange
        incidents = [
            Incident.create("c1", "ABC", "HUMANO", "GRAVE", "El conductor se enveneno", datetime.now()),
            Incident.create("c2", "XYZ", "MECANICO", "LEVE", "El conductor se durmio", datetime.now()),
        ]
    ...
    def test_execute_with_tipo_filter(self, query_incidents_use_case, mock_incident_repository):
        """Given: Type filter, When: Execute, Then: Return filtered incidents."""
        incident = Incident.create("c1", "ABC", "MECANICO", "GRAVE", "El conductor se enveneno", datetime.now())
    ...
    def test_execute_by_id_found(self, query_incidents_use_case, mock_incident_repository):
        """Given: Valid incident ID, When: Query, Then: Return incident."""
        incident_id = uuid4()
        incident = Incident.create("c1", "ABC", "HUMANO", "GRAVE", "El conductor se enveneno", datetime.now())
    ...
    def test_execute_by_id_not_found(self, query_incidents_use_case, mock_incident_repository):
