...
from incidents.domain.models import Incident, IncidentSeverity, IncidentType, PlateNumber
from incidents.infrastructure.adapters.persistence import incident_repository as repository_module
	...
	def filter(self, **kwargs):
		...
		for field, value in kwargs.items():
			if field.endswith("__gte"):
				...
				items = [item for item in items if getattr(item, attribute_name) >= value]
...
class TestDjangoIncidentRepository:
	...
	def _orm_item(self, incident_id="INC-20260601-abcd", plate="ABC-1234", driver="driver-1", tipo="HUMANO", gravedad="GRAVE"):
	...
	def test_find_all_maps_all_records(self):
		FakeIncidentORM.objects = FakeManager([self._orm_item(), self._orm_item(incident_id="INC-20260601-efgh")])
	...
	def test_find_by_placa_filters_records(self):
		FakeIncidentORM.objects = FakeManager(
			[self._orm_item(), self._orm_item(incident_id="INC-20260601-efgh", plate="XYZ-9999")]
		)
	...
	def test_find_by_conductor_filters_records(self):
		FakeIncidentORM.objects = FakeManager(
			[self._orm_item(), self._orm_item(incident_id="INC-20260601-efgh", driver="driver-2")]
		)
	...
	def test_find_by_filters_applies_all_conditions(self):
		FakeIncidentORM.objects = FakeManager(
			[
				self._orm_item(),
				self._orm_item(incident_id="INC-20260601-efgh", plate="XYZ-9999", driver="driver-2", tipo="MECANICO", gravedad="LEVE"),
			]
		)
